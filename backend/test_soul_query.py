...
class TestQueryOrder:
    ...
    def test_third_question_is_subject(self, engine, empty_profile):
        state = QueryState()
        engine.get_next_question(empty_profile, state)  # score
        engine.get_next_question(empty_profile, state)  # province
        q = engine.get_next_question(empty_profile, state)
        assert q is not None
        assert "文科" in q or "理科" in q or "选科" in q

    def test_fourth_question_is_family(self, engine, empty_profile):
        state = QueryState()
        engine.get_next_question(empty_profile, state)  # score
        engine.get_next_question(empty_profile, state)  # province
        engine.get_next_question(empty_profile, state)  # subject
        q = engine.get_next_question(empty_profile, state)
        assert q is not None
        assert "家里" in q or "家庭" in q or "条件" in q
